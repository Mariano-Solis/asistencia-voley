  ...
  async function submit(e) {
    ...
    try {
      if (mode === "player") {
        ...
        if (!data.session) { setMessage("✓ Cuenta Creada. Revisá Tu Correo Para Confirmarla. Al Primer Ingreso Completá Tu Foto De Perfil Para Continuar."); setMode("player"); return; }
        ...
        if (selfie) { const path = `${uid}/${Date.now()}-${selfie.name.replace(/[^a-zA-Z0-9._-]/g, "_")}`; const up = await supabase.storage.from("player-selfies").upload(path, selfie, { upsert: false, contentType: selfie.type || "image/jpeg" }); if (up.error) throw up.error; const linked = await supabase.from("players").update({ selfie_path: path }).eq("id", playerRow.data.id).select("id,selfie_path").single(); if (linked.error || linked.data?.selfie_path !== path) { await supabase.storage.from("player-selfies").remove([path]); throw linked.error || new Error("No Se Pudo Vincular La Foto De Perfil."); } }
