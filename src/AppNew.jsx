...
function AdminUsers({ profile }) {
  ...
  const [editing,setEditing]=useState(null),[editName,setEditName]=useState(""),[editEmail,setEditEmail]=useState(""),[editPassword,setEditPassword]=useState(""),[editActive,setEditActive]=useState(true);
  ...
  function beginEdit(a){setEditing(a);setEditName(a.full_name||"");setEditEmail(a.email||"");setEditPassword("");setEditActive(a.active!==false);setMsg("");}
  ...
  return <section>
    ...
    <div className="professor-management-grid">{admins.length?admins.map(a=><article className="card professor-account-card" key={a.professor_id}>
      <div className="professor-account-head"><div className="professor-account-avatar">👨‍🏫</div><div className="grow"><h3>{a.full_name||"Profe"}</h3><p>{a.email||"Sin Correo Asociado"}</p></div><span className={`professor-status ${a.active&&a.approval_status==="approved"?"active":"pending"}`}>{a.active&&a.approval_status==="approved"?"Activo":"Pendiente"}</span></div>
      <div className="professor-account-details"><div><span>Correo De Cuenta</span><b>{a.email||"—"}</b></div><div><span>Estado</span><b>{a.active?"Activo":"Inactivo"}</b></div><div><span>Aprobación</span><b>{a.approval_status||"—"}</b></div></div>
      <div className="professor-account-actions"><button type="button" onClick={()=>beginEdit(a)}>✏️ Modificar</button><button type="button" className={a.active?"professor-toggle inactive":"professor-toggle active"} disabled={saving} onClick={()=>toggleActive(a)}>{a.active?"⏸️ Inactivar":"▶️ Activar"}</button><button type="button" className="danger" disabled={saving} onClick={()=>removeAdmin(a)}>🗑️ Eliminar</button></div>
...
function Categories({profile,categories,refresh}) {
  const [name,setName]=useState(""),[g,setG]=useState("female"),[msg,setMsg]=useState(""),[editing,setEditing]=useState(null),[draft,setDraft]=useState({name:"",gender:"female",active:true}),[busy,setBusy]=useState(false);
  async function add(e){e.preventDefault();if(!name.trim())return;const r=await supabase.from("categories").insert({name:name.trim(),gender:g,active:true,admin_id:profile.id});if(r.error)setMsg(errorText(r.error));else{setName("");setMsg("✓ Categoría Creada.");await refresh();}}
  if(profile.role!=="super_admin")return null;
  ...
  async function saveEdit(category){
    ...
    try{
      ...
      if(r.error)throw r.error;
      setEditing(null);setMsg("✓ Categoría Modificada Correctamente.");await refresh();
  ...
  async function removeCategory(category){
    ...
    try{
      ...
      setMsg("✓ Categoría Eliminada Correctamente.");await refresh();
    }catch(e){setMsg(errorText(e));}finally{setBusy(false);}
  }
  return <section><PageTitle title="Categorías" text="Creá, Modificá, Inactivá y Eliminá Categorías."/>
    ...
    {msg&&<div className="message">{msg}</div>}
    <div className="category-admin">{categories.map(category=><div className="card category-manage-row" key={category.id}>
...
function App() {
  ...
  const [disabledTabs,setDisabledTabs]=useState([]),[solapasOpen,setSolapasOpen]=useState(false),[solapasDraft,setSolapasDraft]=useState([]),[solapasMsg,setSolapasMsg]=useState(""),[solapasSaving,setSolapasSaving]=useState(false);
  ...
  function clearIdentity() { setSession(null); setProfile(null); setPlayerSession(null); setPlayers([]); setCategories([]); setPermissions({}); setDisabledTabs([]); setSolapasOpen(false); setDualPlayerAvailable(false); setDualPlayerMode(false); }
  ...
  const nav=[['home','Asistencia'],['players','Jugador@s'],['history','Historial'],['schedule','Horarios'],['payments','Pagos'],['requests','Solicitudes']];if(profile.role==='super_admin')nav.push(['admins','Profes'],['categories','Categorías'],['permissions','Permisos']);
  const visibleNav = profile.role === "super_admin" ? nav : nav.filter(([,label])=>!disabledTabs.includes(label));
  async function saveSolapas(){
    setSolapasSaving(true);setSolapasMsg("");
    try{const r=await supabase.from("app_ui_settings").update({disabled_tabs:solapasDraft,updated_at:new Date().toISOString(),updated_by:profile.id}).eq("id","global");if(r.error)throw r.error;setDisabledTabs(solapasDraft);setSolapasMsg("✓ Solapas Actualizadas.");}
    catch(e){setSolapasMsg(errorText(e));}finally{setSolapasSaving(false);}
  }
  return <main className="app"><header className="topbar"><Brand compact/><div className="top-user"><span className="top-user-name">{profile.full_name||"Profe"}</span><span className="role">{profile.role==='super_admin'?'Super Admin':'Profe'}</span>{profile.role==="super_admin"&&<button type="button" className="solapas-launcher" onClick={()=>{setSolapasDraft(disabledTabs);setSolapasMsg("");setSolapasOpen(true)}}><span className="solapas-launcher-icon">⚙️</span><span>Solapas</span></button>}<button onClick={logout}>Salir</button></div></header><nav>{visibleNav.map(([k,l])=><button key={k} data-feature-tab={l} className={tab===k?'active':''} onClick={()=>setTab(k)}>{l}</button>)}{dualPlayerAvailable&&<button type="button" className="player-profile-nav" onClick={()=>setDualPlayerMode(true)}>👤 Mi Perfil</button>}</nav><div className="content"><div className="watermark"/><div className="content-inner">{tab==='home'&&<Attendance profile={profile} players={players} categories={categories} permissions={permissions} refresh={refresh}/>} {tab==='players'&&<Players profile={profile} players={players} categories={categories} permissions={permissions} refresh={refresh}/>} {tab==='history'&&<History profile={profile} players={players} categories={categories} permissions={permissions} refresh={refresh}/>} {tab==='schedule'&&<TrainingSchedule/>} {tab==='payments'&&<AdminPaymentPanel role={profile.role} embedded/>} {tab==='requests'&&<RequestsPage profile={profile}/>} {tab==='admins'&&<AdminUsers profile={profile}/>}  {tab==='categories'&&<Categories profile={profile} categories={categories} refresh={refresh}/>} {tab==='permissions'&&<Permissions profile={profile} categories={categories}/>}</div></div>{solapasOpen&&<div className="modal"><div className="modal-card solapas-modal-card"><div className="modal-head"><div><span className="solapas-eyebrow">⚙️ Personalización</span><h2>Solapas</h2><small>Elegí Qué Secciones Permanecen Visibles Para El Equipo.</small></div><button type="button" onClick={()=>setSolapasOpen(false)}>×</button></div><p>Definí Qué Solapas Ven Profes y Jugador@s. Como Super Admin Siempre Ves Todas.</p><div className="simple-list">{["Asistencia","Jugador@s","Historial","Horarios","Pagos","Solicitudes","Profes","Categorías","Permisos"].map(label=><label className="solapas-option" key={label}><input type="checkbox" checked={!solapasDraft.includes(label)} onChange={e=>setSolapasDraft(cur=>e.target.checked?cur.filter(x=>x!==label):[...cur,label])}/><span>{label}</span></label>)}</div>{solapasMsg&&<div className="message">{solapasMsg}</div>}<div className="form-actions"><button type="button" onClick={()=>setSolapasOpen(false)}>Cerrar</button><button className="primary" type="button" disabled={solapasSaving} onClick={saveSolapas}>{solapasSaving?"Guardando...":"Guardar Cambios"}</button></div></div></div>}<footer><img src={LOGO} alt=""/><span>{APP_NAME} · {TAGLINE}</span></footer></main>;
}
export default App;