...
function App() {
  ...
  function clearIdentity() { setSession(null); setProfile(null); setPlayerSession(null); setPlayers([]); setCategories([]); setPermissions({}); setDisabledTabs([]); setSolapasOpen(false); }
  async function applySession(current) {
    ...
    setPermissions(map); setCategories((c.data||[]).filter(x=>can(p.data,x,map))); setPlayers(nextPlayers); setDisabledTabs(Array.isArray(ui.data?.disabled_tabs)?ui.data.disabled_tabs:[]);
