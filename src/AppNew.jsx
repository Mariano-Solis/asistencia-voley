...
function App() {
  ...
  return <main className="app"><header className="topbar"><Brand compact/><div className="top-user"><span>{profile.full_name||"Profe"}</span><span className="role">{profile.role==='super_admin'?'Super Admin':'Profe'}</span>{profile.role==="super_admin"&&<button type="button" onClick={()=>{setSolapasDraft(disabledTabs);setSolapasMsg("");setSolapasOpen(true)}}>⚙️ Solapas</button>}<button onClick={logout}>Salir</button></div></header><nav>{visibleNav.map(([k,l])=><button key={k} className={tab===k?'active':''} onClick={()=>setTab(k)}>{l}</button>)}</nav><div className="content"><div className="watermark"/><div className="content-inner">{tab==='home'&&<Attendance profile={profile} players={players} categories={categories} permissions={permissions} refresh={refresh}/>} {tab==='players'&&<Players profile={profile} players={players} categories={categories} permissions={permissions} refresh={refresh}/>} {tab==='history'&&<History profile={profile} players={players} categories={categories} permissions={permissions} refresh={refresh}/>} {tab==='schedule'&&<TrainingSchedule/>} {tab==='admins'&&<AdminUsers profile={profile}/>} {tab==='categories'&&<Categories profile={profile} categories={categories} refresh={refresh}/>} {tab==='permissions'&&<Permissions profile={profile} categories={categories}/>}</div></div>{solapasOpen&&<div className="modal"><div className="modal-card"><div className="modal-head"><h2>Solapas</h2><button type="button" onClick={()=>setSolapasOpen(false)}>×</button></div><p>Definí Qué Solapas Ven Profes y Jugador@s. Como Super Admin Siempre Ves Todas.</p><div className="simple-list">{["Asistencia","Jugador@s","Historial","Horarios","Profes","Categorías","Permisos"].map(label=><label className="history-row" key={label}><input type="checkbox" checked={!solapasDraft.includes(label)} onChange={e=>setSolapasDraft(cur=>e.target.checked?cur.filter(x=>x!==label):[...cur,label])}/><b>{label}</b></label>)}</div>{solapasMsg&&<div className="message">{solapasMsg}</div>}<div className="form-actions"><button type="button" onClick={()=>setSolapasOpen(false)}>Cerrar</button><button className="primary" type="button" disabled={solapasSaving} onClick={saveSolapas}>{solapasSaving?"Guardando...":"Guardar Cambios"}</button></div></div></div>}<footer><img src={LOGO} alt=""/><span>{APP_NAME} · {TAGLINE}</span></footer></main>;
}
export default App;